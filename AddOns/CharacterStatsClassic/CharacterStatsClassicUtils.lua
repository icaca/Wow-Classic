...
function CSC_PaperDollFrame_SetMeleeAttackPower(statFrame, unit)
    ...
    CSC_PaperDollFrame_SetLabelAndText(statFrame, "攻强", valueText, false, valueNum);
...
function CSC_PaperDollFrame_SetRangedAttackPower(statFrame, unit)
	...
	if ( PaperDollFrame.noRanged ) then
		print("NO RANGED");
		CSC_PaperDollFrame_SetLabelAndText(statFrame, "攻强", NOT_APPLICABLE, false, 0);
...
function CSC_PaperDollFrame_SetHitChance(statFrame, unit)
	...
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE.."增加攻击命中同等级敌人的几率"..FONT_COLOR_CODE_CLOSE;
	statFrame:Show();
end
...
function CSC_PaperDollFrame_SetSpellHitChance(statFrame, unit)
	...
	local hitChanceText = hitChance;
	CSC_PaperDollFrame_SetLabelAndText(statFrame, "法术命中", hitChanceText, true, hitChance);
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE.."增加法术命中同等级敌人的几率"..FONT_COLOR_CODE_CLOSE;
	statFrame:Show();
end
...
function CSC_PaperDollFrame_SetAttackSpeed(statFrame, unit)
	...
	local displaySpeed = format("%.1F", speed);
	if ( offhandSpeed ) then
		offhandSpeed = format("%.1F", offhandSpeed);
	end
	if ( offhandSpeed ) then
	...
	CSC_PaperDollFrame_SetLabelAndText(statFrame, "攻速", displaySpeed, false, speed);
...
function CSC_PaperDollFrame_SetRangedAttackSpeed(statFrame, unit)
	...
	CSC_PaperDollFrame_SetLabelAndText(statFrame, "攻速", displaySpeed, false, attackSpeed);
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE..format(PAPERDOLLFRAME_TOOLTIP_FORMAT, ATTACK_SPEED).." "..displaySpeed..FONT_COLOR_CODE_CLOSE;
	statFrame:Show();
end
...
function CSC_PaperDollFrame_SetDefense(statFrame, unit)
	...
	local valueNum = max(0, base + posBuff + negBuff);
	CSC_PaperDollFrame_SetLabelAndText(statFrame, "防御", valueText, false, valueNum);
	statFrame.tooltip = tooltipText;
	statFrame.tooltip2 = "减少受到暴击的几率";
	statFrame:Show();
end
...
function CSC_PaperDollFrame_SetSpellPower(statFrame, unit)
	...
	CSC_PaperDollFrame_SetLabelAndText(statFrame, "法强", BreakUpLargeNumbers(minModifier), false, minModifier);
...
function CSC_PaperDollFrame_SetHealing(statFrame, unit)
	...
	CSC_PaperDollFrame_SetLabelAndText(statFrame, "治疗效果", healingText, false, healing);
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE.."提高法术造成的治疗效果"..FONT_COLOR_CODE_CLOSE;
	statFrame:Show();
end
