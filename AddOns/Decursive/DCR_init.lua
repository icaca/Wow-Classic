--[[
    ...
    This file was last updated on 2022-10-20T10:31:27Z
--]]
-------------------------------------------------------------------------------
...
local function InitVariables_Once() -- {{{
    ...
    D.Revision = "7ff1699"; -- not used here but some other add-on may request it from outside
    D.date = "2022-10-26T09:46:29Z";
    D.version = "2.7.8.10";

    if D.date ~= "@project".."-date-iso@" then
        -- 1666777589 doesn't work
...
function D:VersionWarnings(forceDisplay) -- {{{
    ...
    if DcrMaxTOC < T._tocversion then
        ...
        elseif time() - self.db.global.TocExpiredDetection > 3600 * 24 * 40 then -- if more than 40 days elapsed since the detection
            ...
            if time() - self.db.global.LastExpirationAlert > 48 * 3600 or forceDisplay then
                ...
                T._ShowNotice ("|cff00ff00Decursive version: 2.7.8.10|r\n\n" .. "|cFFFFAA66" .. L["TOC_VERSION_EXPIRED"] .. "|r");
    ...
    if (("2.7.8.10"):lower()):find("beta") or ("2.7.8.10"):find("RC") or ("2.7.8.10"):find("Candidate") or alpha then
        ...
        if self.db.global.NonRelease ~= "2.7.8.10" then
            self.db.global.NonRelease = "2.7.8.10";
...
T._LoadedFiles["DCR_init.lua"] = "2.7.8.10";
