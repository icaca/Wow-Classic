...
local normaltext, tradetext = "这些物品只有在你没有携带专业包包的情况下才能自动补货.  它们还会从银行补货(如果你的银行有的话).", "这些物品只有当你带着一个专业包包时才能自动补货.  不会从银行补货."
...
function ns.GenerateRestockPanel(frame)
	...
	local tab1 = tektab.new(frame, "一般", "BOTTOMLEFT", group, "TOPLEFT", 0, -4)
	local tab2 = tektab.new(frame, "专业技能", "LEFT", tab1, "RIGHT", -15, 0)
	tab2:Deactivate()
...
function ns.UpdateConfigList()
	...
	for i,row in ipairs(rows) do
		...
		if id then
			...
			row.stack:SetText("堆叠数: "..(stack or "???"))
			row.icon:Show()
			row:Show()
