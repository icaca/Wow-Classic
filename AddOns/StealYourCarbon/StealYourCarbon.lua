...
function StealYourCarbon:UpgradeWater()
	...
	for _,set in pairs(upgrades) do
		...
		if found and buy > 0 then
			stocklist[found] = buy
			if self.db.chatter then self:PrintF("升级 %s 到 %s", select(2, GetItemInfo(oldid)), select(2, GetItemInfo(found))) end
		end
	end
end
...
function StealYourCarbon:MERCHANT_SHOW()
	...
	for i=1,GetMerchantNumItems() do
		...
		if itemID and stocklist[itemID] then
			...
			if needed > 0 then
				...
				if self.db.chatter then self:PrintF("购买: %s x%d", select(2, GetItemInfo(itemID)), tobuy) end
	...
	if spent > 0 and self.db.chatter then self:Print("花费:", GS(spent)) end
end
