local function loadScoreDB()
	local ScoreDB1 = { "哈霍兰","奥罗","祈福","霜语","席瓦莱恩","碧玉矿洞","怒炉","寒脊山小径","诺格弗格","加丁","毁灭之刃","光芒" }
	local ScoreDB2 = { "莫格莱尼","龙之召唤","奥金斧","灰烬使者","震地者","埃提耶什","维克洛尔","萨弗拉斯","德姆塞卡尔","火锤","水晶之牙","辛迪加" }
	local ScoreDB3 = { "帕奇维克","布鲁","狮心","匕首岭","比格沃斯","审判","比斯巨兽","觅心者","维克尼拉斯","曼多基尔","奎尔塞拉","范沃森" }
	local ScoreDB4 = { "范克瑞斯","雷霆之击","碧空之歌","维希度斯","法尔班克斯","雷德","卓越","巴罗夫","无尽风暴","巨龙追猎者","寒冰之王","秩序之源" }
...
local function CreatPanel()
	...
	introMessageBottom:SetText("数据库"..load_data_info("DATA_VERSION"))
	...
	local updatedateCheckBox = CreateFrame("CheckButton","updatedateCheckBox", configurationPanel, "InterfaceOptionsCheckButtonTemplate")
	updatedateCheckBox:SetPoint("TOPLEFT","tooltipactionBox","TOPLEFT",5,-5)
	_G[updatedateCheckBox:GetName().."Text"]:SetText("鼠标提示显示更新日期")

	local targetdropmenuframeCheckBox = CreateFrame("CheckButton","targetdropmenuframeCheckBox", configurationPanel, "InterfaceOptionsCheckButtonTemplate")
	...
	local targetdropmenuCheckBox = CreateFrame("CheckButton","targetdropmenuCheckBox", configurationPanel, "InterfaceOptionsCheckButtonTemplate")
	...
	local selfmenuCheckBox = CreateFrame("CheckButton","selfmenuCheckBox", configurationPanel, "InterfaceOptionsCheckButtonTemplate")
	...
	local middlebCheckBox = CreateFrame("CheckButton","middlebCheckBox", configurationPanel, "InterfaceOptionsCheckButtonTemplate")
	middlebCheckBox:SetPoint("TOPLEFT","displayscorehotkeyBox","TOPLEFT",5, -5)
	_G[middlebCheckBox:GetName().."Text"]:SetText("鼠标中键")

	local shiftkCheckBox = CreateFrame("CheckButton","shiftkCheckBox", configurationPanel, "InterfaceOptionsCheckButtonTemplate")
	shiftkCheckBox:SetPoint("TOPLEFT","middlebCheckBox","BOTTOMLEFT",0, 0)
	_G[shiftkCheckBox:GetName().."Text"]:SetText("Shift键 + 鼠标右键")

	local controlkCheckBox = CreateFrame("CheckButton","controlkCheckBox", configurationPanel, "InterfaceOptionsCheckButtonTemplate")
	controlkCheckBox:SetPoint("TOPLEFT","shiftkCheckBox","BOTTOMLEFT",0, 0)
	_G[controlkCheckBox:GetName().."Text"]:SetText("Ctrl键 + 鼠标右键")

	local altkCheckBox = CreateFrame("CheckButton","altkCheckBox", configurationPanel, "InterfaceOptionsCheckButtonTemplate")
	...
	local reportscoreCheckBox = CreateFrame("CheckButton","reportscoreCheckBox", configurationPanel, "InterfaceOptionsCheckButtonTemplate")
	...
	local leaderreportCheckBox = CreateFrame("CheckButton","leaderreportCheckBox", configurationPanel, "InterfaceOptionsCheckButtonTemplate")
	...
	local groupscoreinfoCheckBox = CreateFrame("CheckButton","groupscoreinfoCheckBox", configurationPanel, "InterfaceOptionsCheckButtonTemplate")
...
local function insertscore()
	...
	hooksecurefunc("UnitPopup_ShowMenu", function(dropdownMenu, which, unit, name, userData)        
		...
		if wclps_settings.targetdropmenuframe == true or (wclps_settings.targetdropmenu == true and IsAddOnLoaded("tdInspect") == false) then
			if UIDROPDOWNMENU_MENU_LEVEL ~= 2 then
				local function preparebutton()
					...
					else if dstrstop then dstrstop = dstrstop end end end
					local dstrstop2 = load_stop(WP_TargetName.."_2")
					...
					else if dstr then dstr = dstr end end end
					local dstrs2 = cut_strr(load_data(WP_TargetName))
					...
					info.tooltipText = '|cFFFFFF00'..dstrctop..dstrctop2..dstrctop3..dstrstop..dstrstop2..dstrstop3..'|r'..dstr..dstrs2
					UIDropDownMenu_AddSeparator()
					UIDropDownMenu_AddButton(info)
				end
				if wclps_settings.targetdropmenuframe == true then				
					if (which == "SELF" or which == "PLAYER" or which == "PARTY" or which == "RAID_PLAYER" or which == "ENEMY_PLAYER" or which == "TARGET" or which == "FOCUS") and UnitIsPlayer(unit) then			
						preparebutton()
					end
				end
...
function WclPlayerScore:InitCode()
	GameTooltip:HookScript("OnTooltipSetUnit", function(self)
		...
		if UnitExists(unit) and UnitIsPlayer(unit) then
			...
			dstrr = cut_strr(load_data(WP_MouseoverName))
			if dstrr then
				GameTooltip:AddLine(dstrr, 255, 209, 0)
			end
			if dstr then
				dstr = load_data_info("DATA_VERSION")
				if dstr and wclps_settings.updatedate == true then  
					GameTooltip:AddLine(dstr, 255, 209, 0)
				end
			end
			GameTooltip:Show()
	...
	function(self, event, message)
		...
		if IsInGroup() and ( raidmessage or partymessage ) then
			...
			if load_data(WP_SystemName)~= nil then
				...
				local dstrupdate = load_data_info("DATA_VERSION")
				...
				local dstr = cut_str(load_data(WP_SystemName))
				if dstr == nil then dstr = "" else if dstr then dstr = strsub(dstr,11) end end
				local dstrs2 = cut_strr(load_data(WP_SystemName))
				if dstrs2 == nil then dstrs2 = "" else if dstrs2 then dstrs2 = strsub(dstrs2,11) end end			
				SendChatMessage("欢迎"..dstrctop..dstrctop2..dstrctop3..dstrstop..dstrstop2..dstrstop3.." "..dstrplayerclass.."<"..WP_SystemName..">".." "..dstr..dstrs2.." "..dstrupdate, channel)
			end		
		end
	end)
...
averagescoreframe:SetScript("OnEvent",function(self)
	...
	for i = 1, GetNumGroupMembers() do
		local WP_RaidName = select(1, GetRaidRosterInfo(i))
		local dstr = cut_str(load_data(WP_RaidName))
		if dstr == nil or (strmatch(dstr, "神圣") == nil and strmatch(dstr, "恢复") == nil and strmatch(dstr, "戒律") == nil and strmatch(dstr, "梦境") == nil) then 
			dstr = "0:/0" 
	...
	for i = 1, GetNumGroupMembers() do
		local WP_RaidName = select(1, GetRaidRosterInfo(i))
		local dstr = cut_str(load_data(WP_RaidName))
		if dstr == nil or ( strmatch(dstr, "防护") == nil and strmatch(dstr, "正义") == nil and strmatch(dstr, "防战") == nil and strmatch(dstr, "决斗") == nil and strmatch(dstr, "冠军") == nil and strmatch(dstr, "熊") == nil and strmatch(dstr, "典狱长") == nil ) then 
			dstr = "0:/0" 
	...
	local greygreenplayer = SetPlayernumberColor(noscoreandgrey - (GetNumGroupMembers() - raidnumber))
		...
		if tanknumber > 0 then
		GameTooltip:AddLine("注：坦克评分只体现其DPS水平，并不代表其硬度及仇恨能力。",0.8,0.8,0.8,true)
